...
import entity.Room;
import interfaces.*;
import entity.SingleRoom;
import entity.StandardRoom;
import entity.VipSuiteRoom;
import entity.DeluxeRoom;
...
public class RoomManager extends EntityManager implements ModifyObject, PrintSingleObject, PrintAllObjects{
	...
	public RoomManager(Scanner sc, DataAccess dataAccess) {
		...
		if (objArray.length > 0) {	// if there are room objects in the room file
			...
			this.setCounter(roomList.size() + 1);
		}
			...
			for (int i = 0; i < 4; i++) {
				...
				for (int j = 0; j < 12; j++) {
					if (i == 0) {
						roomList.add(new SingleRoom(getCounter(), Room.BedType.SINGLE, Room.AvailabilityStatus.VACANT, true, "nice view", false, 2, roomNumber));
						roomNumber++;
						setCounter(getCounter()+1);
					}
					else if (i == 1) {
						roomList.add(new StandardRoom(getCounter(), Room.BedType.SINGLE, Room.AvailabilityStatus.VACANT, true, "nice view", false, 3, roomNumber));
						roomNumber++;
						setCounter(getCounter()+1);
					}
					else if (i == 2) {
						roomList.add(new VipSuiteRoom(getCounter(), Room.BedType.DOUBLE, Room.AvailabilityStatus.VACANT, true, "nice view", false, 4, roomNumber));
						roomNumber++;
						setCounter(getCounter()+1);
					}
					else {
						roomList.add(new DeluxeRoom(getCounter(), Room.BedType.MASTER, Room.AvailabilityStatus.VACANT, true, "nice view", false, 5, roomNumber));
						roomNumber++;
						setCounter(getCounter()+1);
					}
				}
	...
	public void add() {
		...
		if (rt == 1) {
			roomList.add(new SingleRoom(getCounter(), bt, as, wifiEnabled, facing, smokingAllowed, rLevel, rNumber));
		}
		else if (rt == 2) {
			roomList.add(new StandardRoom(getCounter(), bt, as, wifiEnabled, facing, smokingAllowed, rLevel, rNumber));
		}
		else if (rt == 3) {
			roomList.add(new VipSuiteRoom(getCounter(), bt, as, wifiEnabled, facing, smokingAllowed, rLevel, rNumber));
		}
		else {
